...
class SchemaRetrieval:
    ...
    def __init__(self):
        ...
        local_model_path = "E:/damoxing/AI_Models_Cache/hub/models--BAAI--bge-large-zh-v1.5"
        self.embeddings = HuggingFaceEmbeddings(model_path=local_model_path)
        # 路径配置
    ...
    def _get_schema_version(self) -> str:
       ...
       用于检测表结构是否发生变化。
        ...
        # 1. 获取数据库表结构信息
        schemas = db_manager.get_all_table_schema()
        # 2. 将表结构信息转为字符串
    ...
    def _rewrite_question(self, question: str) -> str:
        ...
        try:
            result = self.question_processor.rewrite(question)
    ...
    def get_relevant_schemas(self, question: str, top_k: int = 5) -> Dict[str, Any]:
        ...
        for table_name in table_names:
            schema = db_manager.get_schema_by_table_name(table_name)
            if schema:
